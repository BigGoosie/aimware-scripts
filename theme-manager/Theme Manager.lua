local outdated, version = 0.1, false
http.Get("https://raw.githubusercontent.com/BigGoosie/aimware-scripts/main/theme-manager/version.txt", function(body)
    if (body ~= version) then 
        outdated = true
        files.Enumerate(function(filename)
            if (GetScriptName() == filename) then
                http.Get("https://raw.githubusercontent.com/BigGoosie/aimware-scripts/main/theme-manager/Theme%20Manager.lua", function(a)
                    file.Write(filename, a)
                end)
